...
const providerTemplates = [
  { label: "GPT 5.5 / 5.4", base: "https://ai.jiegejd.eu.org/v1", model: "gpt-5.5" },
  { label: "VVEAI 全模型", base: "https://api.vveai.com/v1", model: "gpt-4o-mini" },
  { label: "OpenAI 兼容", base: "https://api.openai.com/v1", model: "gpt-4o-mini" },
  { label: "DeepSeek 兼容", base: "https://api.deepseek.com/v1", model: "deepseek-chat" },
]
...
<template>
  <div class="page-stack">
    <section class="hero-panel">
      ...
      <h1 class="hero-title">模型中心：统一管理 OCR 校正与 RAG 问答所需的 LLM 入口。</h1>
      <p class="hero-subtitle">
        页面默认隐藏供应商信息与 API 地址，只保留你自定义的入口名称、模型、用途与价格目录，便于作为产品对外使用。
      </p>
    </section>
    ...
    <NCard class="card-shadow">
      <template #header>
        <div>
          ...
          <p class="section-lead">连接信息会本地持久化保存，界面层默认不暴露供应商地址。</p>
        </div>
      </template>
    ...
    <NModal v-model:show="showForm" preset="card" title="添加模型入口" class="provider-modal">
      <NAlert type="info" class="provider-alert">
        推荐将主力 OCR / RAG 校正模型配置为主路由，其余兼容模型作为回退入口；对外展示时可隐藏供应商信息。
      </NAlert>
      ...
      <NForm label-placement="top">
        <NFormItem label="入口名称">
          <NInput v-model:value="form.name" placeholder="例如 gpt-main / omni-catalog" />
        </NFormItem>
        ...
        <NFormItem label="显示供应商名称">
          <NSpace align="center">
            ...
            <span class="muted">关闭后，页面只展示你的入口名称</span>
          </NSpace>
        </NFormItem>
        <NAlert type="success">
          <template #icon><NIcon><ShieldCheckmark /></NIcon></template>
          保存后 API 地址不会在模型中心页面展示。
        </NAlert>
      </NForm>
