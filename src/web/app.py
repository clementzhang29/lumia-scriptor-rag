"""
ZCLUM 光棱 OCR FastAPI Web 应用 — 主入口文件。
提供 REST API:
...
# 全局状态
_default_data_root = Path(os.environ.get("LOCALAPPDATA", str(Path.home()))) / "ZCLUM Prism OCR"
...
async def index():
    ...
    return HTMLResponse("<h1>ZCLUM Prism OCR API</h1><p>Frontend not built. Run <code>cd frontend && npm install && npm run build</code></p>")
